...
def valid(net, config, test_loader, model_file=None):
    with torch.no_grad():
        ...
        cls_label = []
        cls_pre = []
        temp_predict = torch.zeros((0)).to("cuda:1")
        count = 0
...
if __name__ == "__main__":
    ...
    net = DTCL(input_size = config.len_feature, flag = "Test", a_nums = 60, n_nums = 60)
    net = net.to("cuda:1")
    ...
    valid(net, config, test_loader, model_file = os.path.join(args.model_path, "ucf_best.pkl"))
    end_time = time.time()
